...
- **Trial setup:** Console screenshot now shows a trial Voice number and one selectable destination, but ZGX `.env` still lacks `TWILIO_FROM`, `CS_DEMO_TO_NUMBER`, and `CS_PUBLIC_BASE`. Confirm the intended destination before writing it because it differs from the earlier demo number. Tailscale is installed but Funnel is not enabled.
